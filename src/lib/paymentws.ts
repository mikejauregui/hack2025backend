...
const client = await createAuthenticatedClient({
  walletAddressUrl: iddar_eur, // Make sure the wallet address starts with https:// (not $), and has no trailing slashes
...
const incomingPayment = await client.incomingPayment.create(
  ...
  },
  {
    ...
    incomingAmount: {
      ...
      value: "20", // 100 units of the asset (e.g. cents)
    },
  }
);
...
const outgoingPayment = await client.outgoingPayment.create(
  {
    url: sendingWalletAddress.resourceServer,
    accessToken: "4A30A061352A88D0EB07", // finalizedOutgoingPaymentGrant.access_token.value,
  },
  {
    walletAddress: sendingWalletAddress.id,
    quoteId: quote.id,
